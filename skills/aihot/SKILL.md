...
## 定时任务：ai-news-daily-push

- Cron Job ID: 92114fea4119
- 调度：每天 07:30 CST
- 数据源：aihot（🔥）+ Brave Search（🌊），所有数据必须实时 API 获取，严禁虚构
- 落库：{{数据根目录}}/content/daily-briefing/YYYY-MM-DD.md
- IMA 推送：可选，配置后自动推送到指定笔记本。详见 `references/ima-integration.md`
- 微信推送：摘要版，不放链接。格式模板详见 `templates/wechat-daily.md`
- 索引更新：更新 _index.md
- 关联 Skill：hmworkspace-conventions, ima-skill

### 微信格式规则（用户多次纠正后确定）

- 每板块最多 3 条，5 个板块共约 15 条
- 每板块序号独立从 1 开始（不是全局编号）
- 来源标记在序号右边：1🔥（aihot）/ 2🌊（Brave）
- 标题加粗，紧跟 2-3 句摘要
- 不放链接（省字符）
- 板块间用 ━━━━━━━━━━━━━━━ 分隔，带 emoji 标识
- 洞察和建议用 1️⃣2️⃣3️⃣ 编号
- 开头：☀️ AI 日报 · YYYY-MM-DD
- 如果某板块当天无重要新闻，可跳过

### IMA 格式

- 完整版 Markdown，含所有链接、详细摘要、来源名称、发布日期
- 来源标记用文字：[aihot] / [Brave Search]

### 工作流偏好

- 先跑一遍完整流程让用户确认效果，再建 cron
- 数据必须实时 API 获取，如果某个 API 完全不可用，标注"该数据源今日未获取到数据"，不要编造
- Brave Search 中文覆盖弱，中文 AI 新闻主要靠 aihot
- 网络失败自动切换代理 127.0.0.1:7897 / 直连
